                ...
                const promises = args.CartItems.map(async (obj,index) => {
                    ...
                    let isAlreadyInCart = existingProduct.filter( element => {
                      ...
                      if(isAlreadyInCart.length > 0){
                        ...
                        if(obj.ProductVariantObject === undefined || obj.ProductVariantObject === null )
                        { 
                        ...
                        {   
                            const productVariant = await VariantsSchema.find({ _id: obj.ProductVariantObject._id});
                            totalCartPrice  = totalCartPrice + (productVariant[0].SellingPrice * obj.ProductTotalQuantity);
                            currentCartProduct[index].ProductVariantID = productVariant[0]._id;
                            currentCartProduct[index].ProductTotalQuantity =  currentCartProduct[index].ProductTotalQuantity + obj.ProductTotalQuantity;
