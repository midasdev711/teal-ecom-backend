      ...
      resolve: async (parent, args, context) => {
        ...
        console.log('id', id);
          ...
          if(id.UserID) args.UserID = id.UserID
           ...
           if( typeof lor[0].Description != "undefined" && lor[0].Description != null && lor[0].Description != "" ) {
             lor[0].SubTitle = lor[0].Description.replace( /(<([^>]+)>)/ig, '');
...
// manipulate the data to get the trandending article list
    async function  getAllTrendingArticles( args, limit, offset,ArgsUserID,query ) {
      ...
      return FinalData.map( (title) => {
        // console.log(title);
          if( typeof title.Title != "undefined" && title.Title != null && title.Title.length > TitleMaxLen )
...
async function getUserIntrestData( args,ArgsUserID,sortOptions,offset,limit,query ) {
    ...
    if(UserData != null && UserData.ParentCategories.length > 0 ) {
      ...
      if(filterInrestParent != null && filterInrestSub != null && filterInrestSub.length > 0 || filterInrestParent.length > 0 ) {
// console.log("in leng if");
    ...
    async function IntrestCalculation( IntrestData, limit, offset, args, sortOptions,query ) {
        if( typeof IntrestData != "undefined" && IntrestData != null && IntrestData.length > 0 ) {
          // console.log("in ifff");
          ...
          // query = query.Status
          delete query.ID;
          Data = Articles.find( { $and : [{ID : { $nin : args.ArticleIds }}, query ] } ).sort(sortOptions).skip(offset).limit(limit)
  ...
  return new Promise(async (resolve, reject) => {
    try {
        async.eachSeries(Data, async (data, callback) => {
            ...
            let cauthr = { AuthorID: data["AuthorID"], UserID: ArgsUserID, Status: 1 }

              return Promise.all([ ArticleBookmarks.countDocuments(cquery), FollowAuthor.countDocuments(cauthr), ArticleRatings.countDocuments(cquery) ]).then(function(values) {
...
// get all articles
  ...
  module.exports = { GetPremiumArticles,GetFreeArticles,TrendingArticleList,GetFeaturedImage,MyArticleList,GetAdminArticleList, PopularArticleList,DashboardKeywordSerach,GetParserArticleList,ArticleBySlug,ArticleByID,Article,ArticlesAll };
